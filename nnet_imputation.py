...
class Data:
    ...
    def inputPreprocess(self):
        ...
        self.trainInputs = self.impute_kde(trainData)
        self.testInputs = self.impute_kde(testData)
...
#       self.valInputs = z_score_inputs(self.valInputs,self.trainMean,self.trainStd);
    ...
    def impute_kde(self,data):
        data1 = data.as_matrix()
        r_c = np.shape(data1)
        for col in range(r_c[1]):
            col_idx = np.isnan(data1[:,col])
            minD = np.min(data1[~col_idx,col])
            maxD = np.max(data1[~col_idx,col])
            kernel = gaussian_kde(data1[~col_idx,col])
            nu = np.r_[minD:maxD:1000j][np.argmax(kernel.evaluate(np.r_[minD:maxD:1000j]))]
            data1[:,col][col_idx] = nu
        print(data1)
        return(data1)
